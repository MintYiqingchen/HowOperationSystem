...
BPB_INFO:
	...
	BS_VolLab		DB 'SYSTEM     '; 卷标, 必须 11 个字节（不足补空格）
	BS_FileSysType	DB 'FAT12   '	; 文件系统类型, 必须 8字节
...
LABEL_SEARCH_IN_ROOT_DIR_BEGIN:
	...
	mov	bx, BUFFER_OFFSET	; BX <- OffsetOfLoader（100h）
...
LABEL_FILENAME_FOUND:	; 找到 LOADER.BIN 后便来到这里继续
	...
	mov	ax, SYS_INIT_BASE
	mov	es, ax			; ES <- BaseOfLoader（装载程序基址=9000h）
	mov	bx, 0	; BX <- OffsetOfLoader（装载程序偏移地址=0h）
	mov	ax, cx			; AX <- 起始扇区号
...
; **********************************************************************
	jmp	SYS_INIT_BASE:0	; 这一句正式跳转到已加载到内
						; 存中的 LOADER.BIN 的开始处，
						; 开始执行 LOADER.BIN 的代码。
						; Boot Sector 的使命到此结束
	hlt
...
;字符串
LoaderFileName	db	"HOWOS   SYS", 0 ; LOADER.BIN之文件名
...
times 512*9-3 db 0
;根目录
	V_NAME db 'VolumeA    ';设置卷标
	ATTR db 8
	RETAIN times 10 db 0
	LAST_TIME dw 0x75A0 ;14:45:00
	LAST_DATE dw 0x4A7E ;2017.3.30
	START_CLUSTER dw 0
	FILE_SIZE dd 0;0B